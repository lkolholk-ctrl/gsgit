...
- `data/github/` — слой GitHub API. ✅ Нарезан: `GitHubManager.kt` — чистое сетевое ядро (~420 строк,
  было 9k), домены в `GitHubManager+<Domain>.kt` (31 шт.), модели в `model/` (35 шт.).
  См. `docs/decomposition-log.md`. Расширять — новым доменным файлом по эталону, ядро не трогать.
- `ui/screens/` — Compose-экраны (god-классы: Settings 2.5k, Security 2.2k, ActionsRunDetail 2k,
  ReadmeRenderer 2k, Profile 2k; ViewModel'ов пока нет — стейт живёт в Composable).
...
Прямой `openConnection()` мимо ядра — завести в `GitHubManager.request()` (список сверен 2026-07):
- `ui/screens/RepoReadmeRenderer.kt` — сырой README (кастомный Accept уже поддержан
  через `extraHeaders` ядра — свести туда)
...
- `ui/screens/CodeEditorCopilot.kt` — Copilot `chat/completions` (тоже не GitHub REST; вынести в тот же AI-клиент)

Допустимые исключения (задокументированы в шапках файлов): бинарные стримы в
`GitHubManager+Releases.kt` / `+Workflows.kt` / `+Contents.kt` / `+Diagnostics.kt` —
ядро отдаёт текстовый body и не умеет стримить файлы.
