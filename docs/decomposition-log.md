...
# UI-декомпозиция: CodeEditor (Фаза 1 — разнесение по файлам)

`ui/screens/GitHubCodeEditorModule.kt` (4441 строк). Основной объём — гигантская CodeEditorScreen
(~1620 стр, корень). Та же конвенция (чистое перемещение, двусторонний flip→internal, билд после каждого).

**РЕШЕНИЕ: Фаза 2 CodeEditor отложена под будущий АПГРЕЙД модуля** (решение пользователя). Сейчас —
только Фаза 1. Фазу 2 (ViewModel/стейт CodeEditorScreen) делать вместе с апгрейдом, не отдельно.

## Файл 1: CodeEditorDialogs.kt ✅
FilePickerDialog, ConflictResolverDialog (+парсинг/сборка merge), EditorCompactField, gutter-diff
(713 строк). main: 4441 → 3864. Коллизия имён: CompactField существовал file-private в 3 файлах
...
# СВОДКА ПО ПРОЕКТУ (итог декомпозиции)

**Весь `app/src/main/java`: 160 .kt-файлов, 65 248 строк.**

## Data-слой `data/github/` — 74 файла, 10 710 строк
- `GitHubManager.kt` (ядро): **420 строк** (было 9008 в одном god-файле)
- 33 доменных файла `GitHubManager+*.kt`
- 35 файлов моделей `model/`

## Три нарезанных UI-монолита (Фаза 1)
| Монолит | было (1 файл) | стало | строк сейчас |
|---|---|---|---|
| RepoModule    | 8181 | 9 файлов (core 1462) | 9394 |
| ActionsModule | 5966 | 8 файлов (core 767)  | 7192 |
| CodeEditor    | 4441 | 8 файлов (core 1769) | 5394 |

## Картина в целом
- Было: 4 god-файла (data + 3 UI) = **27 596 строк в 4 файлах**.
- Стало: те же домены/экраны в **~98 файлах**, ядра ужаты на 60–95%.
- Суммарно строк немного выросло (package + дублирующиеся импорты + doc-шапки на файл) —
  нормальная цена модульности: вместо монолитов сфокусированные файлы, новый функционал
  собирается из готовых кусков.

Замер на коммите c621395 (origin/main). Каждый шаг декомпозиции — отдельный коммит с зелёной сборкой.
