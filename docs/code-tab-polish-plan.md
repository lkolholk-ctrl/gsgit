...
- **Фаза 3 — полиш:** ✅ S2 (закрытие редактора при смене ветки), ✅ S4 (recents-персист на диск),
  ✅ S6 (лимит draft-store ~4МБ), ✅ S7 (contentDescription на ключевых иконках).
  - **S1** (commit → bottom-sheet) — **оставлен диалогом сознательно:** с text-input + клавиатурой
    AlertDialog ведёт себя надёжнее кастомного sheet'а; панель изменений — sheet, т.к. без ввода.
  - **S3** (пустые sha/downloadUrl в onOpenPath) — оставлено: для draft-открытия неважно (переоткрытие
    фетчит по path); «фикс» был бы косметикой.
  - **S8** (сырое error-сообщение) — оставлено: тех-сообщение помогает понять причину коммит-ошибки.
  - «Вверх» в крошках — пропущено: крошки навигируют вверх по тапу + жест-назад (избыточно).
